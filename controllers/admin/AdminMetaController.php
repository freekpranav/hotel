...
class AdminMetaControllerCore extends AdminController
{
        ...
        if (!defined('_PS_HOST_MODE_')) {
            if ($this->checkConfiguration($this->ht_file)) {
                ...
                $general_fields['PS_HTACCESS_DISABLE_MODSEC'] = array(
                    ...
                    'hint' => $this->l('Some of Vugha\'s features might not work correctly with a specific configuration of Apache\'s mod_security module. We recommend to turn it off.'),
                    'validation' => 'isBool',
                    'cast' => 'intval',
                    'type' => 'bool',
                );
    ...
    public function generateRobotsFile()
    {
        if (!$write_fd = @fopen($this->rb_file, 'w')) {
            ...
            // PS Comments
            fwrite($write_fd, "# robots.txt automaticaly generated by Vugha e-commerce open-source solution\n");
            fwrite($write_fd, "# https://www.vugha.com \n");
